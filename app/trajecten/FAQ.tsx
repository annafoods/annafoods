...
export default function FAQ() {
  ...
  return (
    <div className="space-y-3">
      {vragen.map((item, i) => (
        <div key={i} className="border border-beige-dark rounded-xl overflow-hidden">
          <button
            className="w-full flex items-center justify-between px-6 py-5 text-left bg-beige hover:bg-beige-dark/40 transition-colors"
            onClick={() => setOpen(open === i ? null : i)}
            aria-expanded={open === i}
          >
            <span className="font-body font-medium text-text-dark pr-4">{item.q}</span>
            <span className={`shrink-0 text-terracotta transition-transform duration-200 ${open === i ? 'rotate-45' : ''}`}>
          ...
          <div className={`overflow-hidden transition-all duration-300 ${open === i ? 'max-h-64' : 'max-h-0'}`}>
            <p className="font-body text-text-medium text-sm leading-relaxed px-6 pb-5 pt-1">
              {item.a}
            </p>
          </div>
        </div>
